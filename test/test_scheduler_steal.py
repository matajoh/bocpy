...
- **Pinned-work stealing** — fan out an independent set of
  behaviours from a single worker so the producer-local arm of
  ``boc_sched_dispatch`` piles them onto one worker's queue. With
  W>1 the idle peers must enter ``boc_sched_steal`` (visible as
  non-zero ``popped_via_steal`` on at least one peer worker) and
...
# ---------------------------------------------------------------------------
# Pinned-work stealing
# ---------------------------------------------------------------------------


class TestStealPinnedWork:
    """Fan-out from one worker forces idle peers to steal."""
    ...
    def test_pinned_fanout_provokes_steals(self):
        """Single kickoff fans out >>BATCH_SIZE work; peers must steal.

        With W=4 and one kickoff that fans out K=400 independent
        behaviours, all K land on the kickoff worker's local queue
        via producer-locality. The other three workers wake (the
        kickoff dispatch and each evict bumps PAUSE_EPOCH and signals
        a peer), find their own queues empty, take the empty-queue
        arm of ``pop_slow``, and call ``boc_sched_steal``. At least
        one peer worker must therefore report
        ``popped_via_steal > 0``; otherwise the steal path is dead.
        """
        _ensure_quiesced()
        W = 4  # noqa: N806
        K = 400  # noqa: N806
        ...
        try:
            ...
            _fanout_kickoff(c_pin, work_cowns, "fanout-done")

            tag, _payload = receive("done", RECEIVE_TIMEOUT)
            assert tag != TIMEOUT, "fan-out kickoff failed to complete"
        ...
        assert total_stolen > 0, (
            f"no worker stole any work despite W={W} and K={K} "
            f"items pinned to one queue: {stats}"
        )
        # And at least one worker must have actually entered the
        # steal path (this is implied by total_stolen > 0 but
        # documents the diagnostic counter explicitly).
        assert total_attempts > 0, (
            f"no steal attempts recorded across {W} workers: {stats}"
        )
...
# ---------------------------------------------------------------------------
# Fairness arm placement (followup)
# ---------------------------------------------------------------------------


def _busy_fanout_step(state, work_cowns, marker, remaining):
    """Tail-recursive busy-worker loop.

    Each iteration fans out a small batch of independent behaviours
    (which pile onto the executing worker's local queue via
    producer-locality) and then re-schedules itself on ``state``.
    The cumulative effect is a worker that drains a long stream of
    locally-queued work without ever pausing — exactly the shape
    the fairness arm is meant to interrupt.
    """
    @when(state)
    def _(state):
        if remaining == 0:
        ...
        for wc in work_cowns:
            ...
            def _(wc):
                wc.value.count += 1
        _busy_fanout_step(state, work_cowns, marker, remaining - 1)


class TestFairnessArmPlacement:
    """The ``should_steal_for_fairness`` arm must fire on busy workers.

    A worker draining a long local stream might never enter
    ``pop_slow``, so the ``should_steal_for_fairness`` flag set by
    token consumption could be observed at most once per quiescent
    gap. ``pop_fast`` therefore routes through ``pop_slow`` whenever
    the flag is set and the local queue is non-empty, matching
    Verona's ``schedulerthread.h::get_work`` placement.
    """
    ...
    def test_busy_worker_honours_fairness_flag(self):
        """A long busy run must fire the fairness arm multiple times.

        Without the fairness routing, the arm fires at most once
        across the entire run (the very first time the worker
        happens to drop into ``pop_slow`` between batches). With
        ``pop_fast`` routing through ``pop_slow`` when the flag is
        set, it fires on every batch where the token has been
        observed, which over a run of ``ITERS`` × ``FANOUT`` items
        should be many.
        """
        _ensure_quiesced()
        W = 4  # noqa: N806
        ITERS = 50  # noqa: N806
        FANOUT = 32   # pile of independent work per iteration  # noqa: N806
        bocpy.start(worker_count=W)
        try:
            state = Cown(_Counter())
            work_cowns = [Cown(_Counter()) for _ in range(FANOUT)]
            _busy_fanout_step(state, work_cowns, "busy-done", ITERS)

            tag, _payload = receive("done", RECEIVE_TIMEOUT)
            assert tag != TIMEOUT, "busy-fanout loop failed to complete"
        ...
        assert len(stats) == W, stats
        total_arm_fires = sum(s["fairness_arm_fires"] for s in stats)
        # Lower bound chosen well above "ran once by accident". With
        # the fix the busy worker fires the arm roughly once per
        # exhausted batch (token re-enqueued each time); ITERS=50 is
        # well above BATCH_SIZE=100 / FANOUT=32 ≈ 3 batches per
        # iteration, so dozens of arm fires are expected.
        assert total_arm_fires >= 5, (
            f"fairness arm fired only {total_arm_fires} times across "
            f"{W} workers over {ITERS}*{FANOUT} items — the busy "
            f"worker is not honouring should_steal_for_fairness: "
            f"{stats}"
        )
