...
import bocpy
from bocpy import Cown, _core, drain, receive, send, set_tags, wait, when
...
def test_wait_returns_final_snapshot():
    ...
    assert sum(s["pushed_local"] + s["pushed_pending"]
...
def test_off_worker_dispatch_bumps_pushed_remote_not_pending():
    ...
    total_pending = sum(s["pushed_pending"] for s in snap)
    assert total_remote >= N, snap
    # No producer-local arm was ever taken, so pending stays at 0.
    assert total_pending == 0, snap


def test_pushed_pending_increments_from_worker_dispatch():
    """A worker-side `@when` against a fresh cown bumps `pushed_pending`.
    ...
    `pushed_pending` (not `pushed_local`). With one chained
    dispatch per outer behavior across N outers, the snapshot
    must show `sum(pushed_pending) >= N`.
    """
    wait()
    W = 2  # noqa: N806
    N = 32  # noqa: N806
