...
class Behaviors:
    ...
    def start(self, module: Optional[tuple[str, str]] = None):
        ...
        # the workers are joined, and by every abort path below
        # (previously, abort paths called `teardown_workers` but never
        # `scheduler_runtime_stop`, leaking the C-side WORKERS array
        # and leaving the next start() observing stale per-task
        # queues).
