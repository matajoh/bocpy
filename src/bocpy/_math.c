...
/// @brief A read/write, storage-sharing 1-D window into a Matrix.
/// @details Holds a strong reference (one IMPL_INCREF) to the ROOT ``base``
///          matrix and describes a single row or column span as an affine
///          range over ``base->data``: element ``i`` lives at
///          ``base->data[offset + i * stride]``. ``offset`` and ``stride`` are
///          signed so a negative-step slice (e.g. ``view[::-1]``) round-trips.
///          A view is read/write by default (or read-only when constructed
///          with ``read_only=True``), pins the whole backing buffer for its
///          lifetime, and is never cross-interpreter shareable.
typedef struct boc_matrix_view {
  PyObject_HEAD matrix_impl *base; ///< strong IMPL_INCREF to the ROOT storage
  Py_ssize_t offset;               ///< first element index into base->data
  size_t count;                    ///< element count (>= 1)
  Py_ssize_t stride;               ///< signed element stride
  bool is_row;                     ///< true => 1 x count, false => count x 1
  bool read_only;                  ///< true => writes through the view raise
} MatrixViewObject;
...
typedef struct {
  ...
  PyTypeObject *view_values_iter_type;
  PyTypeObject *view_indexer_type;
  PyTypeObject *views_iter_type;
...
// ---------------------------------------------------------------------------
// MatrixView: a read/write, storage-sharing 1-D window (row or column) into a
// Matrix. A view holds one strong reference to the ROOT matrix_impl and reads
// through base->data[offset + i*stride]; it is read/write, pins the whole
// backing buffer for its lifetime, and is never cross-interpreter shareable.
//
// view_new and view_materialize sit ahead of unwrap_matrix (which materialises
// a view operand); the rest of the MatrixView surface follows below.
// ---------------------------------------------------------------------------

/// @brief The single guarded constructor for every MatrixView.
/// @details Rejects an empty span (``count == 0``) and validates that both
///          element endpoints (``offset`` and ``offset + (count-1)*stride``)
///          fall inside ``[0, base->size)`` -- the load-bearing guard against
///          an out-of-bounds read. Every caller (row_view / column_view /
///          sub-view / the m.view[...] accessor) builds the span from an
///          in-bounds range and leaves ``stride`` unused when ``count == 1``,
///          so ``(count-1)*stride`` stays within the buffer and cannot
///          overflow. Takes exactly one ``IMPL_INCREF`` on the root ``base``.
///          ``offset``/``stride`` are signed so negative-step slices work.
///          When ``read_only`` is set, writes through the view (item / slice
///          assignment and the ``.x``/``.y``/``.z``/``.w`` setters) raise
///          ``TypeError``; derived views (``.T``, sub-slices) inherit it.
static PyObject *view_new(matrix_impl *base, Py_ssize_t offset, size_t count,
                          Py_ssize_t stride, bool is_row, bool read_only) {
  if (count == 0) {
    PyErr_SetString(PyExc_IndexError, "cannot create an empty view");
    return NULL;
  }
  Py_ssize_t last = offset + (Py_ssize_t)(count - 1) * stride;
  Py_ssize_t size = (Py_ssize_t)base->size;
  if (offset < 0 || offset >= size || last < 0 || last >= size) {
    PyErr_SetString(PyExc_IndexError, "view span out of range");
    return NULL;
  }

  PyTypeObject *type = LOCAL_STATE->view_type;
  MatrixViewObject *view = (MatrixViewObject *)type->tp_alloc(type, 0);
  if (view == NULL) {
    return NULL;
  }
  view->base = base;
...
/// @brief Materialise a view into a fresh, contiguous, owned matrix_impl.
/// @details Guards ownership, then gathers ``count`` strided elements into a
///          new ``1 x count`` (row) or ``count x 1`` (column) buffer. Returns
///          the impl at refcount 0 (the caller wraps or INCREFs it), or NULL
///          with an exception set.
static matrix_impl *view_materialize(MatrixViewObject *v) {
  if (!impl_check_acquired(v->base, true)) {
    return NULL;
  }
  size_t rows = v->is_row ? 1 : v->count;
  size_t cols = v->is_row ? v->count : 1;
  matrix_impl *out = impl_new(rows, cols);
  if (out == NULL) {
    return NULL;
  }
  const double *src = v->base->data;
  double *dst = out->data;
  Py_ssize_t idx = v->offset;
  for (size_t i = 0; i < v->count; ++i, idx += v->stride) {
    dst[i] = src[idx];
  }
  return out;
}
...
matrix_impl *unwrap_matrix(PyObject *op, bool seq_as_column) {
  ...
  // A MatrixView materialises to a fresh contiguous impl so every arithmetic
  // kernel keeps its unit-stride fast path; return it INCREF'd to match the
  // Matrix branch's caller-DECREF contract.
  if (Py_TYPE(op) == LOCAL_STATE->view_type) {
    matrix_impl *out = view_materialize((MatrixViewObject *)op);
    if (out == NULL) {
      return NULL;
    }
    IMPL_INCREF(out);
...
// MatrixView value-access paths (using the view_new / view_materialize helpers
// above). Invariant: every path that reads or writes base storage MUST call
// impl_check_acquired(base, true) before touching data. The pure-metadata
// getters is_row and .T are exempt (no data touch) -- a released view still has
// a valid orientation. A new getter/setter that reads or writes data without
// the guard is a bug.

static PyObject *MatrixView_get_rows(PyObject *op, void *Py_UNUSED(dummy)) {
  MatrixViewObject *v = (MatrixViewObject *)op;
  if (!impl_check_acquired(v->base, true)) {
    return NULL;
  }
  return PyLong_FromSize_t(v->is_row ? 1 : v->count);
}

static PyObject *MatrixView_get_columns(PyObject *op, void *Py_UNUSED(dummy)) {
  MatrixViewObject *v = (MatrixViewObject *)op;
  if (!impl_check_acquired(v->base, true)) {
    return NULL;
  }
  return PyLong_FromSize_t(v->is_row ? v->count : 1);
}

static PyObject *MatrixView_get_size(PyObject *op, void *Py_UNUSED(dummy)) {
  MatrixViewObject *v = (MatrixViewObject *)op;
  if (!impl_check_acquired(v->base, true)) {
    return NULL;
  }
...
static PyObject *MatrixView_get_shape(PyObject *op, void *Py_UNUSED(dummy)) {
  MatrixViewObject *v = (MatrixViewObject *)op;
  if (!impl_check_acquired(v->base, true)) {
    return NULL;
  }
  Py_ssize_t rows = (Py_ssize_t)(v->is_row ? 1 : v->count);
  Py_ssize_t cols = (Py_ssize_t)(v->is_row ? v->count : 1);
  return Py_BuildValue("(nn)", rows, cols);
}

// is_row is pure orientation metadata; no ownership guard (a released view
// still has a well-defined orientation).
static PyObject *MatrixView_get_is_row(PyObject *op, void *Py_UNUSED(dummy)) {
  MatrixViewObject *v = (MatrixViewObject *)op;
  if (v->is_row) {
    Py_RETURN_TRUE;
  }
...
// read_only is pure metadata (no data touch); no ownership guard, matching
// is_row / .T.
static PyObject *MatrixView_get_read_only(PyObject *op,
                                          void *Py_UNUSED(dummy)) {
  if (((MatrixViewObject *)op)->read_only) {
    Py_RETURN_TRUE;
  }
  Py_RETURN_FALSE;
}

// .T is a free (O(1), no-copy) transpose: a new view over the same storage
// with is_row flipped. No data guard -- view_new only reads base->size.
static PyObject *MatrixView_get_T(PyObject *op, void *Py_UNUSED(dummy)) {
  MatrixViewObject *v = (MatrixViewObject *)op;
  return view_new(v->base, v->offset, v->count, v->stride, !v->is_row,
                  v->read_only);
}

// Shared .x/.y/.z/.w component access, matching Matrix's convenience
// accessors: component k is the view's k-th element, at
// base->data[offset + k*stride]. Read/write like the rest of the view: the
// getter guards ownership and the setter writes through to the base. Absent
// components (k >= count) raise IndexError, exactly as on a Matrix.
static PyObject *view_get_component(PyObject *op, size_t k) {
  MatrixViewObject *v = (MatrixViewObject *)op;
  if (!impl_check_acquired(v->base, true)) {
    return NULL;
  }
  ...
  return PyFloat_FromDouble(
      v->base->data[v->offset + (Py_ssize_t)k * v->stride]);
}

static int view_set_component(PyObject *op, PyObject *arg, size_t k) {
  MatrixViewObject *v = (MatrixViewObject *)op;
  if (!impl_check_acquired(v->base, true)) {
    return -1;
  }
  if (v->read_only) {
    PyErr_SetString(PyExc_TypeError, "cannot write to a read-only MatrixView");
    return -1;
  }
  if (v->count <= k) {
    PyErr_SetNone(PyExc_IndexError);
  ...
  // unwrap_double reports a non-number by returning false without setting an
  // exception (it is also used as a type probe elsewhere), so name the error
  // here rather than returning -1 with no exception pending.
  if (!unwrap_double(arg,
                     &v->base->data[v->offset + (Py_ssize_t)k * v->stride])) {
    if (!PyErr_Occurred()) {
      PyErr_SetString(PyExc_TypeError, "expected a real number");
    }
    return -1;
  }
  return 0;
...
static PyObject *MatrixView_get_x(PyObject *op, void *Py_UNUSED(dummy)) {
  return view_get_component(op, 0);
}
static int MatrixView_set_x(PyObject *op, PyObject *arg, void *Py_UNUSED(d)) {
  return view_set_component(op, arg, 0);
}
static PyObject *MatrixView_get_y(PyObject *op, void *Py_UNUSED(dummy)) {
  return view_get_component(op, 1);
}
static int MatrixView_set_y(PyObject *op, PyObject *arg, void *Py_UNUSED(d)) {
  return view_set_component(op, arg, 1);
}
static PyObject *MatrixView_get_z(PyObject *op, void *Py_UNUSED(dummy)) {
  return view_get_component(op, 2);
}
static int MatrixView_set_z(PyObject *op, PyObject *arg, void *Py_UNUSED(d)) {
  return view_set_component(op, arg, 2);
}
static PyObject *MatrixView_get_w(PyObject *op, void *Py_UNUSED(dummy)) {
  return view_get_component(op, 3);
}
static int MatrixView_set_w(PyObject *op, PyObject *arg, void *Py_UNUSED(d)) {
  return view_set_component(op, arg, 3);
}

static Py_ssize_t MatrixView_length(PyObject *op) {
  return (Py_ssize_t)((MatrixViewObject *)op)->count;
}

// A view is 1-D: an int key yields a float, a slice yields a sub-view (composed
// against the root), a tuple / anything else is rejected. Sub-view offset and
// stride are computed with overflow-safe arithmetic (stride may be negative).
static PyObject *MatrixView_subscript(PyObject *op, PyObject *key) {
  MatrixViewObject *v = (MatrixViewObject *)op;
  if (!impl_check_acquired(v->base, true)) {
    return NULL;
  }
  if (PyLong_Check(key)) {
    ...
    if (i < 0 || (size_t)i >= v->count) {
      PyErr_SetString(PyExc_IndexError, "view index out of range");
      return NULL;
    }
    return PyFloat_FromDouble(v->base->data[v->offset + i * v->stride]);
  }
  if (PySlice_Check(key)) {
    range r;
    if (range_read(&r, key, v->count) < 0) {
      return NULL;
    }
    // The stride only matters for a multi-element sub-view; for count <= 1 it
    // is never dereferenced, so avoid the stride*step multiply that a giant
    // slice step could overflow. r.start*stride reaches a valid parent element
    // and stays within the buffer, so it cannot overflow.
    Py_ssize_t new_stride = r.count > 1 ? v->stride * r.step : v->stride;
    Py_ssize_t new_offset = v->offset + r.start * v->stride;
    return view_new(v->base, new_offset, r.count, new_stride, v->is_row,
                    v->read_only);
  }
  PyErr_SetString(PyExc_TypeError,
                  "MatrixView is 1-D; index with a single int or slice");
  return NULL;
}

// Item / slice assignment writes THROUGH the shared storage of a read/write
// view (a read-only view rejects the write with TypeError up front). An int
// key writes one scalar; a slice key broadcasts a
// scalar or copies a length-matched sequence / Matrix / view. Matching is
// length-only -- a view is 1-D, so RHS orientation is irrelevant. A view cannot
// resize, so deletion (value == NULL) is rejected. The ownership guard is the
// same one the reads use, so writing an unowned view raises RuntimeError
// symmetrically. The step*stride multiply is guarded behind count > 1 (unused
// for a single element), matching the read sub-view path.
static int MatrixView_ass_subscript(PyObject *op, PyObject *key,
                                    PyObject *value) {
  MatrixViewObject *v = (MatrixViewObject *)op;
  if (!impl_check_acquired(v->base, true)) {
    return -1;
  ...
  if (value == NULL) {
    PyErr_SetString(PyExc_TypeError, "cannot delete elements of a MatrixView");
    return -1;
  }
  ...
  if (PyLong_Check(key)) {
    ...
    v->base->data[v->offset + i * v->stride] = scalar;
    return 0;
  }
  ...
  if (PySlice_Check(key)) {
    ...
    Py_ssize_t step = r.count > 1 ? v->stride * r.step : v->stride;
    Py_ssize_t start = v->offset + r.start * v->stride;

    // A scalar RHS broadcasts to every selected element.
    if (PyLong_Check(value) || PyFloat_Check(value)) {
      ...
      for (size_t k = 0; k < r.count; ++k, idx += step) {
        v->base->data[idx] = scalar;
      }
      return 0;
    }

    // Otherwise the RHS is a sequence / Matrix / view: length-only match.
    matrix_impl *rhs = unwrap_matrix(value, false);
    if (rhs == NULL) {
      return -1;
    }
    if (rhs->size != r.count) {
      PyErr_Format(PyExc_ValueError,
                   "cannot assign %zu values to a view of length %zu",
                   rhs->size, r.count);
      IMPL_DECREF(rhs);
      return -1;
    }
...
// repr/str delegate to a materialised temporary Matrix (via runtime str()),
// prefixed so a human sees it is a borrow. Degrade (never raise) on a released
// base, matching Matrix's "<not acquired>" contract.
static PyObject *view_render(PyObject *op) {
  MatrixViewObject *v = (MatrixViewObject *)op;
  if (!impl_check_acquired(v->base, false)) {
    return PyUnicode_FromString("MatrixView(<not acquired>)");
  }
  matrix_impl *out = view_materialize(v);
  if (out == NULL) {
    return NULL;
  }
  PyObject *tmp = wrap_impl_or_free(out);
  if (tmp == NULL) {
    return NULL;
  }
  PyObject *inner = PyObject_Str(tmp);
  Py_DECREF(tmp);
  if (inner == NULL) {
    return NULL;
  }
  PyObject *res = PyUnicode_FromFormat("MatrixView(%U)", inner);
  Py_DECREF(inner);
  return res;
}

static PyObject *MatrixView_repr(PyObject *op) { return view_render(op); }
static PyObject *MatrixView_str(PyObject *op) { return view_render(op); }

// Lazy strided iterator returned by MatrixView.values(): a strong ref to the
// view plus a cursor. Re-checks ownership every step (the base may be released
// between yields), mirroring MatrixValuesIter.
typedef struct {
  PyObject_HEAD MatrixViewObject *view;
  size_t cursor;
} MatrixViewValuesIterObject;

static void MatrixViewValuesIter_dealloc(PyObject *op) {
  MatrixViewValuesIterObject *it = (MatrixViewValuesIterObject *)op;
...
static PyObject *MatrixViewValuesIter_next(PyObject *op) {
  MatrixViewValuesIterObject *it = (MatrixViewValuesIterObject *)op;
  MatrixViewObject *v = it->view;
  if (!impl_check_acquired(v->base, true)) {
    return NULL;
  }
  if (it->cursor >= v->count) {
    return NULL; // StopIteration
  }
  double value = v->base->data[v->offset + (Py_ssize_t)it->cursor * v->stride];
  it->cursor++;
  return PyFloat_FromDouble(value);
}

static PyObject *MatrixViewValuesIter_self(PyObject *op) {
  Py_INCREF(op);
  return op;
}

// A view iterator is only ever born through MatrixView.values(); direct
// construction would leave view == NULL and NULL-deref on first step.
static PyObject *MatrixViewValuesIter_new(PyTypeObject *Py_UNUSED(type),
                                          PyObject *Py_UNUSED(args),
                                          PyObject *Py_UNUSED(kwds)) {
  PyErr_SetString(PyExc_TypeError,
                  "cannot create 'bocpy._math.MatrixViewValuesIter' instances "
                  "directly; use MatrixView.values()");
  return NULL;
}

static PyType_Slot MatrixViewValuesIter_slots[] = {
    {Py_tp_new, MatrixViewValuesIter_new},
    {Py_tp_dealloc, MatrixViewValuesIter_dealloc},
    {Py_tp_iter, MatrixViewValuesIter_self},
    {Py_tp_iternext, MatrixViewValuesIter_next},
    {0, NULL},
};

static PyType_Spec MatrixViewValuesIter_Spec = {
    .name = "bocpy._math.MatrixViewValuesIter",
    .basicsize = sizeof(MatrixViewValuesIterObject),
    .itemsize = 0,
    .flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_IMMUTABLETYPE,
    .slots = MatrixViewValuesIter_slots};

static PyObject *MatrixView_values(PyObject *op, PyObject *Py_UNUSED(ignored)) {
  MatrixViewObject *v = (MatrixViewObject *)op;
  if (!impl_check_acquired(v->base, true)) {
    return NULL;
  }
  PyTypeObject *type = LOCAL_STATE->view_values_iter_type;
  MatrixViewValuesIterObject *it =
      (MatrixViewValuesIterObject *)type->tp_alloc(type, 0);
  if (it == NULL) {
    return NULL;
  }
  Py_INCREF(op);
...
// A view is 1-D, so iterating it yields its elements (like a 1-D array), not
// sub-views: `for x in view` is exactly `for x in view.values()`. Delegates to
// MatrixView_values so both share one lazy, ownership-re-checking iterator.
static PyObject *MatrixView_iter(PyObject *op) {
  return MatrixView_values(op, NULL);
}

// Materialise the view into an independent, owned Matrix (canonical type). The
// result shares no storage with the base, so mutating either leaves the other
// unchanged -- the escape hatch for shipping a view across interpreters.
static PyObject *MatrixView_copy(PyObject *op, PyObject *Py_UNUSED(ignored)) {
  matrix_impl *out = view_materialize((MatrixViewObject *)op);
  if (out == NULL) {
    return NULL;
  }
  return (PyObject *)wrap_impl_or_free(out);
}

// A view aliases its base's storage and pins the whole backing buffer, so it
// is deliberately not shippable. Refuse pickling eagerly (at dumps time) with
// a message that names the .copy() remedy -- overriding __reduce__ takes
// precedence over object.__reduce_ex__, so this fires for pickle.dumps and for
// every bocpy path that serialises (Cown payloads, send()).
static PyObject *MatrixView_reduce(PyObject *Py_UNUSED(op),
                                   PyObject *Py_UNUSED(ignored)) {
  PyErr_SetString(PyExc_TypeError,
                  "cannot pickle or send a MatrixView across interpreters; "
                  "call .copy() to get an owned Matrix");
  return NULL;
}

static PyMethodDef MatrixView_methods[] = {
    {"values", MatrixView_values, METH_NOARGS,
     "values($self, /)\n--\n\n"
     "Return a lazy iterator over the view's elements."},
    {"copy", MatrixView_copy, METH_NOARGS,
     "copy($self, /)\n--\n\n"
     "Return an independent Matrix holding a copy of the view's elements."},
    {"__reduce__", MatrixView_reduce, METH_NOARGS,
     "__reduce__($self, /)\n--\n\n"
     "Refuse pickling; a view aliases its base and cannot be shipped. "
     "Call .copy() to get an owned, shippable Matrix."},
    {NULL} /* Sentinel */
};

static PyGetSetDef MatrixView_getset[] = {
    {"rows", (getter)MatrixView_get_rows, NULL, NULL, NULL},
    {"columns", (getter)MatrixView_get_columns, NULL, NULL, NULL},
    {"size", (getter)MatrixView_get_size, NULL, NULL, NULL},
    {"shape", (getter)MatrixView_get_shape, NULL, NULL, NULL},
    {"is_row", (getter)MatrixView_get_is_row, NULL, NULL, NULL},
    {"read_only", (getter)MatrixView_get_read_only, NULL, NULL, NULL},
    {"T", (getter)MatrixView_get_T, NULL, NULL, NULL},
    {"x", (getter)MatrixView_get_x, (setter)MatrixView_set_x, NULL, NULL},
    {"y", (getter)MatrixView_get_y, (setter)MatrixView_set_y, NULL, NULL},
    {"z", (getter)MatrixView_get_z, (setter)MatrixView_set_z, NULL, NULL},
    {"w", (getter)MatrixView_get_w, (setter)MatrixView_set_w, NULL, NULL},
    {NULL} /* Sentinel */
};

// A MatrixView is only ever born through view_new (via Matrix.row_view /
// column_view / view[...]); direct construction (and pickle __newobj__
// reconstruction) would leave base == NULL and NULL-deref on first use.
static PyObject *MatrixView_new(PyTypeObject *Py_UNUSED(type),
                                PyObject *Py_UNUSED(args),
                                PyObject *Py_UNUSED(kwds)) {
  PyErr_SetString(PyExc_TypeError,
                  "cannot create 'bocpy._math.MatrixView' instances directly; "
                  "use Matrix.row_view() / Matrix.column_view()");
  return NULL;
}

static void MatrixView_dealloc(PyObject *op) {
  MatrixViewObject *self = (MatrixViewObject *)op;
  PyTypeObject *type = Py_TYPE(op);
  IMPL_DECREF(self->base);
  type->tp_free(op);
  Py_DECREF(type);
}

// A view participates in arithmetic by materialising at the unwrap_matrix
// boundary, so the binary and matmul number slots reuse Matrix's own
// operand-symmetric kernels verbatim (they never dereference a left operand's
// impl -- both operands go through unwrap_matrix). Forward-declared here; the
// macro-stamped definitions live further down with the Matrix number protocol.
static PyObject *Matrix_Add_op(PyObject *lhs, PyObject *rhs);
static PyObject *Matrix_Subtract_op(PyObject *lhs, PyObject *rhs);
static PyObject *Matrix_Multiply_op(PyObject *lhs, PyObject *rhs);
static PyObject *Matrix_Divide_op(PyObject *lhs, PyObject *rhs);
static PyObject *Matrix_matmul(PyObject *lhs_op, PyObject *rhs_op);

// Unary ops cannot reuse Matrix_Negate_op / Matrix_Abs_op: those blind-cast
// their operand to MatrixObject and read self->impl. Instead materialise the
// view to an owned Matrix and delegate to its number protocol.
static PyObject *view_unary(PyObject *op, PyObject *(*fn)(PyObject *)) {
  matrix_impl *out = view_materialize((MatrixViewObject *)op);
  if (out == NULL) {
    return NULL;
  }
  ...
  PyObject *result = fn(tmp);
  Py_DECREF(tmp);
  return result;
}

static PyObject *MatrixView_negative(PyObject *op) {
  return view_unary(op, PyNumber_Negative);
}

static PyObject *MatrixView_absolute(PyObject *op) {
  return view_unary(op, PyNumber_Absolute);
}

static PyType_Slot MatrixView_slots[] = {
    {Py_tp_new, MatrixView_new},
    {Py_tp_dealloc, MatrixView_dealloc},
    {Py_tp_getset, MatrixView_getset},
    {Py_tp_methods, MatrixView_methods},
    {Py_mp_subscript, MatrixView_subscript},
    {Py_mp_ass_subscript, MatrixView_ass_subscript},
    {Py_mp_length, MatrixView_length},
    {Py_sq_length, MatrixView_length},
    {Py_tp_iter, MatrixView_iter},
    {Py_tp_repr, MatrixView_repr},
    {Py_tp_str, MatrixView_str},
    // Binary / matmul reuse Matrix's operand-symmetric kernels; the view is
    // materialised at unwrap_matrix. Item/slice assignment writes through
    // (Py_mp_ass_subscript); there are deliberately no Py_nb_inplace_* slots,
    // so ``view += x`` rebinds the name to a fresh Matrix rather than mutating.
    ...
    {Py_nb_negative, MatrixView_negative},
    {Py_nb_absolute, MatrixView_absolute},
    {0, NULL},
};

static PyType_Spec MatrixView_Spec = {.name = "bocpy._math.MatrixView",
                                      .basicsize = sizeof(MatrixViewObject),
                                      .itemsize = 0,
                                      .flags = Py_TPFLAGS_DEFAULT |
                                               Py_TPFLAGS_IMMUTABLETYPE,
                                      .slots = MatrixView_slots};

// MatrixViewIndexer: the object returned by Matrix.view, giving m.view[...]
// slice-notation construction of MatrixViews. Holds a strong ref to the source
// Matrix; guards ownership at __getitem__ time (so m.view itself is cheap).
typedef struct {
  PyObject_HEAD MatrixObject *matrix;
} MatrixViewIndexerObject;

static PyObject *MatrixViewIndexer_new(PyTypeObject *Py_UNUSED(type),
                                       PyObject *Py_UNUSED(args),
                                       PyObject *Py_UNUSED(kwds)) {
  PyErr_SetString(PyExc_TypeError,
                  "cannot create 'bocpy._math.MatrixViewIndexer' instances "
                  "directly; use Matrix.view");
  return NULL;
}

static void MatrixViewIndexer_dealloc(PyObject *op) {
  MatrixViewIndexerObject *ix = (MatrixViewIndexerObject *)op;
  PyTypeObject *type = Py_TYPE(op);
  Py_XDECREF(ix->matrix);
  type->tp_free(op);
  Py_DECREF(type);
}

static PyObject *MatrixViewIndexer_subscript(PyObject *op, PyObject *key) {
  MatrixViewIndexerObject *ix = (MatrixViewIndexerObject *)op;
  matrix_impl *impl = ix->matrix->impl;
  if (!impl_check_acquired(impl, true)) {
    return NULL;
  }
  const Py_ssize_t columns = (Py_ssize_t)impl->columns;

  // 1-D int key -> the whole row as a row view (== m.view[i, :]).
  if (PyLong_Check(key)) {
    Py_ssize_t r = PyLong_AsSsize_t(key);
    if (r == -1 && PyErr_Occurred()) {
      return NULL;
    }
    if (r < 0) {
      r += (Py_ssize_t)impl->rows;
    }
    if (r < 0 || (size_t)r >= impl->rows) {
      PyErr_SetString(PyExc_IndexError, "row index out of range");
      return NULL;
    }
    return view_new(impl, r * columns, impl->columns, 1, true, false);
  }

  // 2-tuple (row_key, col_key): orientation from which axis is a bare int.
  if (PyTuple_Check(key) && PyTuple_GET_SIZE(key) == 2) {
    range rr;
    range cc;
    if (range_read(&rr, PyTuple_GET_ITEM(key, 0), impl->rows) < 0) {
      return NULL;
    }
    if (range_read(&cc, PyTuple_GET_ITEM(key, 1), impl->columns) < 0) {
      return NULL;
    }
    // Base offset of the first selected cell is always a valid flat index
    // in [0, size), so it cannot overflow.
    Py_ssize_t base_off = rr.start * columns + cc.start;
    if (rr.scalar && cc.scalar) {
      return view_new(impl, base_off, 1, 1, true, false); // 1-element row view
    }
    if (rr.scalar) {
      return view_new(impl, base_off, cc.count, cc.step, true,
                      false); // row view
    }
    if (cc.scalar) {
      // Column view: stride matters only for count > 1 (where |rr.step| < rows,
      // so columns*rr.step < size and cannot overflow); for count <= 1 the
      // stride is unused, so skip the multiply a giant step could overflow.
      Py_ssize_t stride = rr.count > 1 ? columns * rr.step : columns;
      return view_new(impl, base_off, rr.count, stride, false,
                      false); // column view
    }
    PyErr_SetString(PyExc_TypeError,
                    "a view must be a single row or column; use m[...] to copy "
                    "a submatrix");
    return NULL;
  }

  PyErr_SetString(PyExc_TypeError,
                  "a view must be a single row or column; use m[...] to copy a "
                  "submatrix");
  return NULL;
}

static PyType_Slot MatrixViewIndexer_slots[] = {
    {Py_tp_new, MatrixViewIndexer_new},
    {Py_tp_dealloc, MatrixViewIndexer_dealloc},
    {Py_mp_subscript, MatrixViewIndexer_subscript},
    {0, NULL},
};

static PyType_Spec MatrixViewIndexer_Spec = {
    .name = "bocpy._math.MatrixViewIndexer",
    .basicsize = sizeof(MatrixViewIndexerObject),
    .itemsize = 0,
    .flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_IMMUTABLETYPE,
    .slots = MatrixViewIndexer_slots};

// Decode an optional slice/None into a full range over `length`. None (axis
// omitted) yields the whole axis; a slice decodes via range_read (steps and
// negative steps honoured). Returns 0 on success, -1 with an exception set.
static int view_axis_range(PyObject *key, size_t length, range *out) {
  if (key == NULL || key == Py_None) {
    out->start = 0;
    out->stop = (Py_ssize_t)length;
    out->step = 1;
    out->count = length;
    out->scalar = false;
    return 0;
  }
  return range_read(out, key, length);
}

static PyObject *Matrix_row_view(PyObject *op, PyObject *args, PyObject *kwds) {
  MatrixObject *self = (MatrixObject *)op;
  matrix_impl *impl = self->impl;
  Py_ssize_t r = 0;
  PyObject *cols = NULL;
  int read_only = 0;
  static char *kwlist[] = {"row", "columns", "read_only", NULL};
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "n|O$p", kwlist, &r, &cols,
                                   &read_only)) {
    return NULL;
  }
  if (!impl_check_acquired(impl, true)) {
  ...
  range c;
  if (view_axis_range(cols, impl->columns, &c) < 0) {
    return NULL;
  }
  Py_ssize_t offset = r * (Py_ssize_t)impl->columns + c.start;
  return view_new(impl, offset, c.count, c.step, true, (bool)read_only);
}

static PyObject *Matrix_column_view(PyObject *op, PyObject *args,
                                    PyObject *kwds) {
  MatrixObject *self = (MatrixObject *)op;
  matrix_impl *impl = self->impl;
  Py_ssize_t col = 0;
  PyObject *rows = NULL;
  int read_only = 0;
  static char *kwlist[] = {"column", "rows", "read_only", NULL};
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "n|O$p", kwlist, &col, &rows,
                                   &read_only)) {
    return NULL;
  }
  if (!impl_check_acquired(impl, true)) {
  ...
  if (col < 0) {
    col += (Py_ssize_t)impl->columns;
  }
  if (col < 0 || (size_t)col >= impl->columns) {
    PyErr_SetString(PyExc_IndexError, "column index out of range");
    return NULL;
  }
  range rr;
  if (view_axis_range(rows, impl->rows, &rr) < 0) {
    return NULL;
  }
  Py_ssize_t offset = rr.start * (Py_ssize_t)impl->columns + col;
  // Column view: stride matters only for count > 1 (where |rr.step| < rows, so
  // columns*rr.step < size and cannot overflow); for count <= 1 it is unused,
  // so skip the multiply that a giant slice step could otherwise overflow.
  Py_ssize_t stride = rr.count > 1 ? (Py_ssize_t)impl->columns * rr.step
                                   : (Py_ssize_t)impl->columns;
  return view_new(impl, offset, rr.count, stride, false, (bool)read_only);
}

static PyObject *Matrix_get_view(PyObject *op, void *Py_UNUSED(dummy)) {
  // Cheap: no ownership guard here -- a stale accessor fails at __getitem__.
  PyTypeObject *type = LOCAL_STATE->view_indexer_type;
  MatrixViewIndexerObject *ix =
      (MatrixViewIndexerObject *)type->tp_alloc(type, 0);
  if (ix == NULL) {
    return NULL;
  }
  Py_INCREF(op);
  ix->matrix = (MatrixObject *)op;
  return (PyObject *)ix;
}

// Iterator returned by Matrix.row_views() / column_views(): yields a FRESH
// MatrixView for each row (is_row) or column, one at a time. Holds a strong
// ref to the source matrix and re-checks ownership every step. Each yielded
// view is a distinct object -- never a reused cursor -- so list(m.row_views())
// and comprehensions behave like any other iterator.
typedef struct {
  PyObject_HEAD MatrixObject *matrix;
  size_t cursor;
  bool is_row;
  bool read_only;
} MatrixViewsIterObject;

static void MatrixViewsIter_dealloc(PyObject *op) {
  MatrixViewsIterObject *it = (MatrixViewsIterObject *)op;
  PyTypeObject *type = Py_TYPE(op);
...
static PyObject *MatrixViewsIter_next(PyObject *op) {
  MatrixViewsIterObject *it = (MatrixViewsIterObject *)op;
  matrix_impl *impl = it->matrix->impl;
  if (!impl_check_acquired(impl, true)) {
    return NULL;
  }
  size_t limit = it->is_row ? impl->rows : impl->columns;
  if (it->cursor >= limit) {
    return NULL; // StopIteration
  }
  Py_ssize_t columns = (Py_ssize_t)impl->columns;
  size_t i = it->cursor++;
  if (it->is_row) {
    // Row i occupies the contiguous span [i*columns, i*columns + columns).
    return view_new(impl, (Py_ssize_t)i * columns, impl->columns, 1, true,
                    it->read_only);
  }
  // Column i is strided by `columns` down the rows starting at flat index i.
  return view_new(impl, (Py_ssize_t)i, impl->rows, columns, false,
                  it->read_only);
}

static PyObject *MatrixViewsIter_self(PyObject *op) {
  Py_INCREF(op);
  return op;
}

// A views iterator is only ever born through Matrix.row_views() /
// column_views(); direct construction would leave matrix == NULL and
// NULL-deref on first step.
static PyObject *MatrixViewsIter_new(PyTypeObject *Py_UNUSED(type),
                                     PyObject *Py_UNUSED(args),
                                     PyObject *Py_UNUSED(kwds)) {
  PyErr_SetString(PyExc_TypeError,
                  "cannot create 'bocpy._math.MatrixViewsIter' instances "
                  "directly; use Matrix.row_views() / Matrix.column_views()");
  return NULL;
}

static PyType_Slot MatrixViewsIter_slots[] = {
    {Py_tp_new, MatrixViewsIter_new},
    {Py_tp_dealloc, MatrixViewsIter_dealloc},
    {Py_tp_iter, MatrixViewsIter_self},
    {Py_tp_iternext, MatrixViewsIter_next},
    {0, NULL},
};

static PyType_Spec MatrixViewsIter_Spec = {
    .name = "bocpy._math.MatrixViewsIter",
    .basicsize = sizeof(MatrixViewsIterObject),
    .itemsize = 0,
    .flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_IMMUTABLETYPE,
    .slots = MatrixViewsIter_slots};

static PyObject *make_views_iter(PyObject *op, bool is_row, bool read_only) {
  MatrixObject *self = (MatrixObject *)op;
  if (!impl_check_acquired(self->impl, true)) {
    return NULL;
  }
  PyTypeObject *type = LOCAL_STATE->views_iter_type;
  MatrixViewsIterObject *it = (MatrixViewsIterObject *)type->tp_alloc(type, 0);
  if (it == NULL) {
    return NULL;
  }
  Py_INCREF(op);
  ...
  it->cursor = 0;
  it->is_row = is_row;
  it->read_only = read_only;
  return (PyObject *)it;
}

static PyObject *Matrix_row_views(PyObject *op, PyObject *args,
                                  PyObject *kwds) {
  int read_only = 0;
  static char *kwlist[] = {"read_only", NULL};
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "|$p", kwlist, &read_only)) {
    return NULL;
  }
  return make_views_iter(op, true, (bool)read_only);
}

static PyObject *Matrix_column_views(PyObject *op, PyObject *args,
                                     PyObject *kwds) {
  int read_only = 0;
  static char *kwlist[] = {"read_only", NULL};
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "|$p", kwlist, &read_only)) {
  ...
  return make_views_iter(op, false, (bool)read_only);
}
...
/// @brief The "no valid index" sentinel written by the bin / digitize kernels
///        for a NaN input cell. Mirrors the argextreme masked-empty convention
///        (-1): a NaN value has no meaningful bin, so it maps to -1 rather than
///        a valid ``[0, n]`` index. Stored as a double in the result Matrix
///        (read back as ``-1.0``).
#define INDEX_SENTINEL (-1.0)

/// @brief Data range ignoring NaN, mirroring the masked-aggregate skip rule.
/// @details ``v < lo`` / ``v > hi`` are both false for a NaN ``v``, so NaN
///          never updates either bound. An all-NaN (or empty) matrix leaves
///          ``lo > hi`` (DBL_MAX / -DBL_MAX), which callers treat as a
///          degenerate range.
static void impl_data_range(const matrix_impl *m, double *lo_out,
                            double *hi_out) {
  double lo = DBL_MAX;
  double hi = -DBL_MAX;
  const double *p = m->data;
...
/// @brief Equal-width binning kernel: map each value to its bin index.
/// @details Bins partition ``[lo, hi]`` into ``n`` equal-width intervals.
///          ``idx = floor((v - lo) * n / (hi - lo))``; ``right`` uses
///          ``ceil(...) - 1`` so a value on an interior boundary drops into
///          the lower bin. The index is clamped to ``[0, n-1]`` (so ``v ==
///          hi`` lands in the last bin). A NaN value maps to the -1 "no valid
///          index" sentinel (matching the argextreme masked-empty convention).
///          A degenerate range (``hi <= lo`` -- every value equal) sends each
///          non-NaN value to bin 0. Indices are published as doubles (Matrix
///          stores only doubles). Each output cell depends solely on the
///          co-located input cell plus the pre-computed range, so an in-place
///          run (``out`` aliasing the source) is safe.
static void impl_bin_ewise(const matrix_impl *m, matrix_impl *out, double lo,
                           double hi, long n, bool right) {
  const double range = hi - lo;
  const double scale = range > 0.0 ? (double)n / range : 0.0;
  const double top = (double)(n - 1);
  const double *sp = m->data;
  double *dp = out->data;
  for (size_t i = 0; i < m->size; ++i, ++sp, ++dp) {
    const double v = *sp;
    ...
    const double pos = (v - lo) * scale;
    double idx = right ? ceil(pos) - 1.0 : floor(pos);
    if (idx < 0.0) {
      idx = 0.0;
    } else if (idx > top) {
      idx = top;
...
/// @brief Digitize kernel: map each value to the index of its edge interval.
/// @details ``edges`` is a strictly increasing array of ``nedges`` interior
///          boundaries. With ``right`` false the index is the count of edges
///          ``<= v`` (bin ``i`` means ``edges[i-1] <= v < edges[i]``); with
///          ``right`` true it is the count of edges ``< v`` (``edges[i-1] < v
///          <= edges[i]``), matching ``numpy.digitize``. Indices span ``[0,
///          nedges]``. A NaN value maps to the -1 "no valid index" sentinel.
///          The scan is linear in ``nedges``: edge counts are expected to be
///          far smaller than the cell count, keeping it below the
///          binary-search crossover. A large-histogram fast path can be added
///          behind a flag later.
static void impl_digitize_ewise(const matrix_impl *m, matrix_impl *out,
                                const double *edges, size_t nedges,
                                bool right) {
  const double *sp = m->data;
  double *dp = out->data;
  for (size_t i = 0; i < m->size; ++i, ++sp, ++dp) {
...
/// @brief Map every element to its equal-width bin index over a range.
/// @details ``bins`` (positional, must be >= 1) is the number of equal-width
///          bins spanning the range. ``bounds`` is an optional ``(min, max)``
///          tuple that fixes the range and skips the O(size) data scan; when
///          omitted (``None``) the range is the matrix's own min/max, both
///          computed ignoring NaN. A value outside a fixed range clamps into
///          the nearest edge bin (0 or ``bins-1``). Output routing matches the
///          unary elementwise family via ``set_output``: ``in_place=True``
///          bins ``self`` in place and returns it, the keyword-only ``out``
///          writes into a caller-supplied same-shape matrix, and the two are
///          mutually exclusive. ``right`` selects the boundary side. See
///          ``impl_bin_ewise`` for the index formula and the NaN /
///          degenerate-range handling. The range is resolved before
///          ``set_output`` so an in-place run bins against the original
///          values.
static PyObject *Matrix_bin(PyObject *op, PyObject *args, PyObject *kwds) {
  MatrixObject *self = (MatrixObject *)op;
  matrix_impl *impl = self->impl;

  long bins = 0;
  PyObject *bounds_op = Py_None;
  int right = 0;
  int in_place = 0;
  PyObject *out_target = NULL;
  static char *kwlist[] = {"bins", "bounds", "right", "in_place", "out", NULL};

  if (!PyArg_ParseTupleAndKeywords(args, kwds, "l|$OppO", kwlist, &bins,
                                   &bounds_op, &right, &in_place,
                                   &out_target)) {
    return NULL;
  }
  ...
  if (bins < 1) {
    PyErr_SetString(PyExc_ValueError, "bins must be >= 1");
    return NULL;
  }
  ...
  // Resolve the binning range BEFORE set_output: an in-place run makes out
  // alias impl, and each cell must bin against the original range, not a
  // partially overwritten buffer. A `bounds=(lo, hi)` pair fixes the range and
  // skips the O(size) scan; bounds=None falls back to the matrix's own min/max
  // (ignoring NaN).
  double lo = 0.0;
  double hi = 0.0;
  if (bounds_op == Py_None) {
    impl_data_range(impl, &lo, &hi);
  } else {
    PyObject *seq =
        PySequence_Fast(bounds_op, "bounds must be a (min, max) pair");
    if (seq == NULL) {
      return NULL;
    }
    if (PySequence_Fast_GET_SIZE(seq) != 2) {
      Py_DECREF(seq);
      PyErr_SetString(PyExc_ValueError,
                      "bounds must be a (min, max) pair of length 2");
      return NULL;
    }
    if (!unwrap_double(PySequence_Fast_GET_ITEM(seq, 0), &lo) ||
        !unwrap_double(PySequence_Fast_GET_ITEM(seq, 1), &hi)) {
      Py_DECREF(seq);
      PyErr_SetString(PyExc_TypeError, "bounds values must be real numbers");
      return NULL;
    }
    Py_DECREF(seq);
    if (!isfinite(lo) || !isfinite(hi)) {
      PyErr_SetString(PyExc_ValueError, "bounds values must be finite");
      return NULL;
    }
    if (hi < lo) {
      PyErr_SetString(PyExc_ValueError, "bounds max must be >= min");
      return NULL;
    }
  }
  ...
  impl_bin_ewise(impl, out, lo, hi, bins, (bool)right);
  return out_op;
}

/// @brief Map every element to the index of the edge interval it falls in.
/// @details ``edges`` (positional) is a strictly increasing vector of interior
///          boundaries -- a Matrix vector (1xN or Mx1) or any sequence that
///          ``unwrap_matrix`` coerces. Output routing matches the unary
///          elementwise family via ``set_output``: ``in_place`` and the
///          keyword-only ``out`` are mutually exclusive. ``right`` selects the
///          boundary side (``numpy.digitize`` convention). See
///          ``impl_digitize_ewise`` for the index rule and NaN handling.
static PyObject *Matrix_digitize(PyObject *op, PyObject *args, PyObject *kwds) {
  MatrixObject *self = (MatrixObject *)op;
  matrix_impl *impl = self->impl;

  PyObject *edges_op = NULL;
  int right = 0;
  int in_place = 0;
  PyObject *out_target = NULL;
  static char *kwlist[] = {"edges", "right", "in_place", "out", NULL};

  if (!PyArg_ParseTupleAndKeywords(args, kwds, "O|$ppO", kwlist, &edges_op,
                                   &right, &in_place, &out_target)) {
    return NULL;
  }
  ...
  matrix_impl *edges = unwrap_matrix(edges_op, false);
  if (edges == NULL) {
    return NULL;
  }

  // A vector of any orientation reads as a flat contiguous boundary list;
  // reject a genuine 2-D matrix rather than silently flattening it.
  if (edges->rows != 1 && edges->columns != 1) {
    PyErr_SetString(PyExc_ValueError, "edges must be a vector");
    IMPL_DECREF(edges);
    return NULL;
  }

  // Strictly increasing and NaN-free: both the count-of-edges index rule and
  // the scan's early-out assume monotone boundaries.
  for (size_t i = 0; i < edges->size; ++i) {
    if (isnan(edges->data[i]) ||
        (i > 0 && edges->data[i] <= edges->data[i - 1])) {
      PyErr_SetString(PyExc_ValueError,
                      "edges must be strictly increasing and non-NaN");
      IMPL_DECREF(edges);
      return NULL;
    }
  }

  // Snapshot edges into an owned buffer so the kernel is decoupled from the
  // source object: without this, m.digitize(m, in_place=True) would read
  // boundaries out of the very buffer it is overwriting. Edge counts are small
  // by design, so the copy is negligible.
  size_t nedges = edges->size;
  double *edge_buf = PyMem_RawMalloc(nedges * sizeof(double));
  if (edge_buf == NULL) {
    IMPL_DECREF(edges);
    PyErr_NoMemory();
    return NULL;
  }
  memcpy(edge_buf, edges->data, nedges * sizeof(double));
  IMPL_DECREF(edges);

  PyObject *out_op = NULL;
  matrix_impl *out = set_output(impl, op, &out_op, out_target, in_place);
  if (out == NULL) {
    PyMem_RawFree(edge_buf);
    return NULL;
  }

  impl_digitize_ewise(impl, out, edge_buf, nedges, (bool)right);
  PyMem_RawFree(edge_buf);
  return out_op;
}
...
// Matrix-side factory materialising a MatrixView into an independent, owned
// Matrix -- the classmethod twin of MatrixView.copy(), sharing view_materialize
// and fitting the Matrix.zeros / ones / full / vector / concat factory family.
static PyObject *Matrix_from_view(PyObject *Py_UNUSED(cls), PyObject *view_op) {
  if (Py_TYPE(view_op) != LOCAL_STATE->view_type) {
    PyErr_Format(PyExc_TypeError, "from_view expects a MatrixView, not %.200s",
                 Py_TYPE(view_op)->tp_name);
    return NULL;
  }
  matrix_impl *out = view_materialize((MatrixViewObject *)view_op);
  if (out == NULL) {
    return NULL;
  }
  return (PyObject *)wrap_impl_or_free(out);
...
static PyGetSetDef Matrix_getset[] = {
    ...
    {"view", (getter)Matrix_get_view, NULL, NULL, NULL},
    {NULL} /* Sentinel */
};
...
static PyMethodDef Matrix_methods[] = {
    {"row_view", (PyCFunction)Matrix_row_view, METH_VARARGS | METH_KEYWORDS,
     "row_view($self, /, row, columns=None, *, read_only=False)\n--\n\n"
     ...
     "slicing) rejects writes with ``TypeError``."},
    {"column_view", (PyCFunction)Matrix_column_view,
     METH_VARARGS | METH_KEYWORDS,
     "column_view($self, /, column, rows=None, *, read_only=False)\n--\n\n"
    ...
    {"row_views", (PyCFunction)Matrix_row_views, METH_VARARGS | METH_KEYWORDS,
     "row_views($self, /, *, read_only=False)\n--\n\n"
     ...
     "distinct object. Pass ``read_only=True`` for immutable row views."},
    {"column_views", (PyCFunction)Matrix_column_views,
     METH_VARARGS | METH_KEYWORDS,
     "column_views($self, /, *, read_only=False)\n--\n\n"
    ...
    {"argmax", (PyCFunction)Matrix_argmax_method, METH_VARARGS | METH_KEYWORDS,
     ...
     "included element yields -1, the integer analog of a masked min/max\n"
     "returning NaN."},
    {"bin", (PyCFunction)Matrix_bin, METH_VARARGS | METH_KEYWORDS,
     "bin($self, /, bins, *, bounds=None, right=False, "
     "in_place=False, out=None)\n--\n\n"
     ...
     "(min, max) pair of finite numbers with max >= min."},
    {"digitize", (PyCFunction)Matrix_digitize, METH_VARARGS | METH_KEYWORDS,
     "digitize($self, /, edges, *, right=False, in_place=False, out=None)\n"
     "--\n\n"
     ...
     "keyword-only ``out`` are mutually exclusive."},
    ...
    {"sqrt", (PyCFunction)Matrix_Sqrt_method, METH_VARARGS | METH_KEYWORDS,
     ...
     "(a same-shape matrix) the result is written there and returned; "
     "``out`` and ``in_place`` are mutually exclusive."},
    {"reciprocal", (PyCFunction)Matrix_Reciprocal_method,
     METH_VARARGS | METH_KEYWORDS,
     "reciprocal($self, /, *, in_place=False, out=None)\n--\n\n"
     "Element-wise reciprocal (``1 / x``). Zero elements yield +/-inf. With "
     "``out`` (a same-shape matrix) the result is written there and "
     "returned; ``out`` and ``in_place`` are mutually exclusive."},
    ...
    {"copy", Matrix_copy, METH_NOARGS,
     "copy($self, /)\n--\n\nReturn a deep copy."},
    {"from_view", Matrix_from_view, METH_O | METH_CLASS,
     "from_view($type, view, /)\n--\n\n"
     "Materialise a MatrixView into a new, independent Matrix (the "
     "classmethod twin of MatrixView.copy())."},
...
static int _math_module_exec(PyObject *module) {
  ...
  state->view_type =
      (PyTypeObject *)PyType_FromModuleAndSpec(module, &MatrixView_Spec, NULL);
  if (state->view_type == NULL) {
    return -1;
  }

  if (PyModule_AddType(module, state->view_type) < 0) {
    return -1;
  }
  // MatrixView is intentionally NOT passed to XIDATA_REGISTERCLASS: a view is a
  // local borrow of its base matrix and cannot cross interpreters (a view is
  // not shippable). Callers .copy() to obtain an owned Matrix to ship.

  // The two companion types are private (NOT PyModule_AddType'd), like
  // values_iter_type: the values() iterator and the m.view[...] accessor.
  state->view_values_iter_type = (PyTypeObject *)PyType_FromModuleAndSpec(
      module, &MatrixViewValuesIter_Spec, NULL);
  if (state->view_values_iter_type == NULL) {
    return -1;
  }

  state->view_indexer_type = (PyTypeObject *)PyType_FromModuleAndSpec(
      module, &MatrixViewIndexer_Spec, NULL);
  if (state->view_indexer_type == NULL) {
    return -1;
  }

  state->views_iter_type = (PyTypeObject *)PyType_FromModuleAndSpec(
      module, &MatrixViewsIter_Spec, NULL);
  if (state->views_iter_type == NULL) {
    return -1;
  }
...
static int _math_module_clear(PyObject *module) {
  ...
  Py_CLEAR(state->view_values_iter_type);
  Py_CLEAR(state->view_indexer_type);
  Py_CLEAR(state->views_iter_type);
...
static int _math_module_traverse(PyObject *module, visitproc visit, void *arg) {
  ...
  Py_VISIT(state->view_values_iter_type);
  Py_VISIT(state->view_indexer_type);
  Py_VISIT(state->views_iter_type);
