...
static PyObject *_core_noticeboard_write_direct(PyObject *self,
  ...
  if (noticeboard_write(key, key_len, xidata, is_pickled,
                        new_pins, new_pin_count) < 0) {
    return NULL;
  }
...
static PyObject *_core_notice_sync_complete(PyObject *self, PyObject *args) {
  ...
  notice_sync_complete((int_least64_t)seq);
...
static PyObject *_core_notice_sync_wait(PyObject *self, PyObject *args) {
  ...
  bool ok;
  Py_BEGIN_ALLOW_THREADS
  ok = notice_sync_wait((int_least64_t)my_seq, timeout, wait_forever);
  Py_END_ALLOW_THREADS
...
static PyObject *_core_terminator_wait(PyObject *self, PyObject *args) {
  ...
  bool ok;
  Py_BEGIN_ALLOW_THREADS
  ok = terminator_wait(timeout, wait_forever);
  Py_END_ALLOW_THREADS
  ...
  if (qptr == NULL) {
    ...
    if (!PyErr_Occurred()) {
      PyErr_Format(PyExc_KeyError,
                   "No queue available for tag %R: tag capacity exceeded",
                   tag);
    }
    return NULL;
  }
...
static int boc_enqueue(BOCMessage *message) {
  ...
  while (true) {
    ...
    if (atomic_compare_exchange_strong(&qptr->tail, &tail, tail + 1)) {
      ...
      boc_atomic_fetch_add_u64_explicit(&qptr->pushed_total, 1,
                                        BOC_MO_RELAXED);
  ...
  *message = NULL;
  ...
  while (head < tail) {
    ...
    *message = qptr->messages[index];
    ...
    boc_atomic_fetch_add_u64_explicit(&qptr->popped_total, 1,
                                      BOC_MO_RELAXED);
...
static PyObject *_core_scheduler_stats(PyObject *Py_UNUSED(module),
  ...
  for (Py_ssize_t i = 0; i < n; ++i) {
    ...
    if (boc_sched_stats_snapshot(i, &s) < 0) {
      PyErr_SetString(PyExc_RuntimeError,
                      "boc_sched_stats_snapshot failed");
      Py_DECREF(result);
      return NULL;
    }
    PyObject *d = Py_BuildValue(
        ...
        "enqueue_cas_retries", (unsigned long long)s.enqueue_cas_retries,
        "dequeue_cas_retries", (unsigned long long)s.dequeue_cas_retries,
        "batch_resets", (unsigned long long)s.batch_resets,
        "steal_attempts", (unsigned long long)s.steal_attempts,
        "steal_failures", (unsigned long long)s.steal_failures,
        "parked", (unsigned long long)s.parked,
        "last_steal_attempt_ns",
        (unsigned long long)s.last_steal_attempt_ns,
        "fairness_arm_fires", (unsigned long long)s.fairness_arm_fires);
    if (d == NULL) {
      Py_DECREF(result);
      return NULL;
    }
...
static PyObject *_core_queue_stats(PyObject *Py_UNUSED(module),
  ...
  BOCQueue *qptr = BOC_QUEUES;
  for (size_t i = 0; i < BOC_QUEUE_COUNT; ++i, ++qptr) {
    int_least64_t state = atomic_load_explicit(&qptr->state,
                                               memory_order_relaxed);
    if (state != BOC_QUEUE_ASSIGNED) {
      continue;
    }
    BOCTag *tag = (BOCTag *)atomic_load_explicit(&qptr->tag,
                                                 memory_order_relaxed);
    PyObject *tag_obj;
    ...
    uint64_t deq_r = boc_atomic_load_u64_explicit(&qptr->dequeue_cas_retries,
                                                  BOC_MO_RELAXED);
    uint64_t pushed = boc_atomic_load_u64_explicit(&qptr->pushed_total,
                                                   BOC_MO_RELAXED);
    uint64_t popped = boc_atomic_load_u64_explicit(&qptr->popped_total,
                                                   BOC_MO_RELAXED);
    PyObject *d = Py_BuildValue(
        "{s:n,s:N,s:K,s:K,s:K,s:K}",
        "queue_index", (Py_ssize_t)qptr->index,
        "tag", tag_obj, // steals ref
        "enqueue_cas_retries", (unsigned long long)enq_r,
        "dequeue_cas_retries", (unsigned long long)deq_r,
        "pushed_total", (unsigned long long)pushed,
...
static PyObject *_core_scheduler_runtime_start(PyObject *Py_UNUSED(module),
  ...
  for (Py_ssize_t i = 0; i < (Py_ssize_t)n; ++i) {
    BOCBehavior *token =
        (BOCBehavior *)PyMem_RawCalloc(1, sizeof(BOCBehavior));
    if (token == NULL) {
...
static PyObject *_core_scheduler_worker_register(
    PyObject *Py_UNUSED(module), PyObject *Py_UNUSED(dummy)) {
...
/// @param Py_UNUSED
/// @return Py_None.
static PyObject *_core_scheduler_request_stop_all(
    PyObject *Py_UNUSED(module), PyObject *Py_UNUSED(dummy)) {
...
///
/// **Why not in @c boc_sched_shutdown.** Releasing the cown chain\n/// requires Python-level @c release_all (it touches @c BOCRequest\n/// arrays whose freeing routes through @c COWN_DECREF). Doing this\n/// in C without the GIL would also deadlock against any pending\n/// noticeboard mutator. The Python orchestration layer is the right\n/// place to coordinate.\n/// @param module The _core module\n/// @param Py_UNUSED unused arg\n/// @return Fresh @c list[BehaviorCapsule] (possibly empty), or NULL\n///         on allocation failure with a Python exception set.
static PyObject *_core_scheduler_drain_all_queues(
    PyObject *Py_UNUSED(module), PyObject *Py_UNUSED(dummy)) {
