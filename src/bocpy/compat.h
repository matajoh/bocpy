...
// ===========================================================================
// Typed `boc_atomic_*_explicit` API.
//
// The legacy `atomic_*` polyfill above is implicit-barrier on MSVC and was
// only ever called with `memory_order_seq_cst` (which expanded to 0 and was
// ignored). The work-stealing scheduler and the C2 two-epoch CAS protocol
// need ordering-correct acquire/release/relaxed semantics. On x86/x64 the
// implicit-TSO assumption that makes the legacy API accidentally correct
// does NOT hold on ARM64 Windows, so we need a typed surface that picks the
// right `Interlocked*_{acq,rel,nf}` variant per memory order.
//
// On POSIX the typed API is a thin wrapper around `<stdatomic.h>` — the
// compiler folds it away. Legacy `atomic_*` callers are unaffected; the new
// API is purely additive.
// ===========================================================================

#ifdef _WIN32

// MSVC: `volatile T` storage with distinct typedefs per width so the
...
// Memory-order tags. Distinct integer constants so the MSVC dispatch can
// `switch` on them; on POSIX they are mapped to `memory_order_*` by the
// `boc_mo_to_std` helper below. Skip 1 to leave room for `consume`.
typedef enum {
  BOC_MO_RELAXED = 0,
  BOC_MO_ACQUIRE = 2,
  BOC_MO_RELEASE = 3,
...
#ifndef _WIN32
// ---------- POSIX implementation: static-inline wrappers --------------------
...
#define BOC_ATOMIC_OPS_(SUF, T, AT)                                            \
  ...
  static inline void boc_atomic_store_##SUF##_explicit(                        \
      AT *p, T v, boc_memory_order_t o) {                                      \
    atomic_store_explicit(p, v, boc_mo_to_std(o));                             \
  }                                                                            \
  static inline T boc_atomic_exchange_##SUF##_explicit(                        \
      AT *p, T v, boc_memory_order_t o) {                                      \
...
#else
// ---------- MSVC implementation: switch on order, dispatch to Interlocked* --
//
// On x86/x64 every Interlocked* intrinsic is a full barrier, so all
// orderings collapse to the unsuffixed form (which is correct for any
// requested ordering). On ARM64 we pick the matching `_acq`/`_rel`/`_nf`
// variant. `BOC_MO_ACQ_REL` and `BOC_MO_SEQ_CST` use the unsuffixed (full
// barrier) form on every target.

#if defined(_M_ARM64)
#define BOC_IL_LOAD64_ACQ(p) ((uint64_t)__ldar64((unsigned __int64 const volatile *)(p)))
#define BOC_IL_LOAD32_ACQ(p) ((uint32_t)__ldar32((unsigned __int32 const volatile *)(p)))
#define BOC_IL_LOAD8_ACQ(p) ((uint8_t)__ldar8((unsigned __int8 const volatile *)(p)))
#define BOC_IL_STORE64_REL(p, v) __stlr64((unsigned __int64 volatile *)(p), (unsigned __int64)(v))
#define BOC_IL_STORE32_REL(p, v) __stlr32((unsigned __int32 volatile *)(p), (unsigned __int32)(v))
#define BOC_IL_STORE8_REL(p, v) __stlr8((unsigned __int8 volatile *)(p), (unsigned __int8)(v))
#endif

// ---- u64 -------------------------------------------------------------------

static inline uint64_t
boc_atomic_load_u64_explicit(boc_atomic_u64_t *p, boc_memory_order_t order) {
#if defined(_M_ARM64)
  switch (order) {
  case BOC_MO_RELAXED: return *p;
  case BOC_MO_ACQUIRE: return BOC_IL_LOAD64_ACQ(p);
  default:             return BOC_IL_LOAD64_ACQ(p);
  }
#else
  (void)order;
  return *p;
...
static inline void
boc_atomic_store_u64_explicit(boc_atomic_u64_t *p, uint64_t v,
                              boc_memory_order_t order) {
#if defined(_M_ARM64)
  switch (order) {
  case BOC_MO_RELAXED: *p = v; return;
  case BOC_MO_RELEASE: BOC_IL_STORE64_REL(p, v); return;
  default: (void)_InterlockedExchange64((volatile __int64 *)p, (__int64)v); return;
  }
#else
  (void)order;
  *p = v;
...
#if defined(_M_ARM64)
  switch (order) {
  case BOC_MO_RELAXED: return (uint64_t)_InterlockedExchange64_nf((volatile __int64 *)p, (__int64)v);
  case BOC_MO_ACQUIRE: return (uint64_t)_InterlockedExchange64_acq((volatile __int64 *)p, (__int64)v);
  case BOC_MO_RELEASE: return (uint64_t)_InterlockedExchange64_rel((volatile __int64 *)p, (__int64)v);
  default:             return (uint64_t)_InterlockedExchange64((volatile __int64 *)p, (__int64)v);
  }
#else
  (void)order;
...
static inline bool
boc_atomic_compare_exchange_strong_u64_explicit(
  ...
  switch (succ) {
  case BOC_MO_RELAXED:
    prev = (uint64_t)_InterlockedCompareExchange64_nf((volatile __int64 *)p, (__int64)desired, (__int64)exp);
    break;
  case BOC_MO_ACQUIRE:
    prev = (uint64_t)_InterlockedCompareExchange64_acq((volatile __int64 *)p, (__int64)desired, (__int64)exp);
    break;
  case BOC_MO_RELEASE:
    prev = (uint64_t)_InterlockedCompareExchange64_rel((volatile __int64 *)p, (__int64)desired, (__int64)exp);
    break;
  default:
    prev = (uint64_t)_InterlockedCompareExchange64((volatile __int64 *)p, (__int64)desired, (__int64)exp);
    break;
  }
#else
  (void)succ;
...
#if defined(_M_ARM64)
  switch (order) {
  case BOC_MO_RELAXED: return (uint64_t)_InterlockedExchangeAdd64_nf((volatile __int64 *)p, (__int64)v);
  case BOC_MO_ACQUIRE: return (uint64_t)_InterlockedExchangeAdd64_acq((volatile __int64 *)p, (__int64)v);
  case BOC_MO_RELEASE: return (uint64_t)_InterlockedExchangeAdd64_rel((volatile __int64 *)p, (__int64)v);
  default:             return (uint64_t)_InterlockedExchangeAdd64((volatile __int64 *)p, (__int64)v);
  }
#else
  (void)order;
...
boc_atomic_load_u32_explicit(boc_atomic_u32_t *p, boc_memory_order_t order) {
#if defined(_M_ARM64)
  switch (order) {
  case BOC_MO_RELAXED: return *p;
  case BOC_MO_ACQUIRE: return BOC_IL_LOAD32_ACQ(p);
  default:             return BOC_IL_LOAD32_ACQ(p);
  }
#else
  (void)order;
  return *p;
...
static inline void
boc_atomic_store_u32_explicit(boc_atomic_u32_t *p, uint32_t v,
                              boc_memory_order_t order) {
#if defined(_M_ARM64)
  switch (order) {
  case BOC_MO_RELAXED: *p = v; return;
  case BOC_MO_RELEASE: BOC_IL_STORE32_REL(p, v); return;
  default: (void)_InterlockedExchange((volatile long *)p, (long)v); return;
  }
#else
  (void)order;
  *p = v;
...
#if defined(_M_ARM64)
  switch (order) {
  case BOC_MO_RELAXED: return (uint32_t)_InterlockedExchange_nf((volatile long *)p, (long)v);
  case BOC_MO_ACQUIRE: return (uint32_t)_InterlockedExchange_acq((volatile long *)p, (long)v);
  case BOC_MO_RELEASE: return (uint32_t)_InterlockedExchange_rel((volatile long *)p, (long)v);
  default:             return (uint32_t)_InterlockedExchange((volatile long *)p, (long)v);
  }
#else
  (void)order;
...
static inline bool
boc_atomic_compare_exchange_strong_u32_explicit(
  ...
  switch (succ) {
  case BOC_MO_RELAXED:
    prev = (uint32_t)_InterlockedCompareExchange_nf((volatile long *)p, (long)desired, (long)exp);
    break;
  case BOC_MO_ACQUIRE:
    prev = (uint32_t)_InterlockedCompareExchange_acq((volatile long *)p, (long)desired, (long)exp);
    break;
  case BOC_MO_RELEASE:
    prev = (uint32_t)_InterlockedCompareExchange_rel((volatile long *)p, (long)desired, (long)exp);
    break;
  default:
    prev = (uint32_t)_InterlockedCompareExchange((volatile long *)p, (long)desired, (long)exp);
    break;
  }
#else
  (void)succ;
...
#if defined(_M_ARM64)
  switch (order) {
  case BOC_MO_RELAXED: return (uint32_t)_InterlockedExchangeAdd_nf((volatile long *)p, (long)v);
  case BOC_MO_ACQUIRE: return (uint32_t)_InterlockedExchangeAdd_acq((volatile long *)p, (long)v);
  case BOC_MO_RELEASE: return (uint32_t)_InterlockedExchangeAdd_rel((volatile long *)p, (long)v);
  default:             return (uint32_t)_InterlockedExchangeAdd((volatile long *)p, (long)v);
  }
#else
  (void)order;
...
static inline bool
boc_atomic_load_bool_explicit(boc_atomic_bool_t *p, boc_memory_order_t order) {
#if defined(_M_ARM64)
  switch (order) {
  case BOC_MO_RELAXED: return (bool)*p;
  case BOC_MO_ACQUIRE: return (bool)BOC_IL_LOAD8_ACQ(p);
  default:             return (bool)BOC_IL_LOAD8_ACQ(p);
  }
#else
  (void)order;
  return (bool)*p;
...
static inline void
boc_atomic_store_bool_explicit(boc_atomic_bool_t *p, bool v,
                               boc_memory_order_t order) {
#if defined(_M_ARM64)
  switch (order) {
  case BOC_MO_RELAXED: *p = (uint8_t)v; return;
  case BOC_MO_RELEASE: BOC_IL_STORE8_REL(p, (uint8_t)v); return;
  default: (void)_InterlockedExchange8((volatile char *)p, (char)v); return;
  }
#else
  (void)order;
  *p = (uint8_t)v;
...
static inline bool
boc_atomic_exchange_bool_explicit(boc_atomic_bool_t *p, bool v,
                                  boc_memory_order_t order) {
  (void)order;
...
static inline bool
boc_atomic_compare_exchange_strong_bool_explicit(
    boc_atomic_bool_t *p, bool *expected, bool desired,
    boc_memory_order_t succ, boc_memory_order_t fail) {
  (void)succ;
  (void)fail;
  char exp = (char)*expected;
  char prev = _InterlockedCompareExchange8((volatile char *)p, (char)desired, exp);
  if (prev == exp) return true;
  *expected = (bool)prev;
  return false;
}
...
static inline void *
boc_atomic_load_ptr_explicit(boc_atomic_ptr_t *p, boc_memory_order_t order) {
...
static inline void
boc_atomic_store_ptr_explicit(boc_atomic_ptr_t *p, void *v,
                              boc_memory_order_t order) {
#if defined(_M_ARM64)
...
static inline void *
boc_atomic_exchange_ptr_explicit(boc_atomic_ptr_t *p, void *v,
                                 boc_memory_order_t order) {
  (void)order;
...
static inline bool
boc_atomic_compare_exchange_strong_ptr_explicit(
    boc_atomic_ptr_t *p, void **expected, void *desired,
    boc_memory_order_t succ, boc_memory_order_t fail) {
  (void)succ;
  (void)fail;
...
#endif // _WIN32 / POSIX dispatch

// ---------------------------------------------------------------------------
// Timeout validation helper
// ---------------------------------------------------------------------------
//
