"""Fanout microbenchmark for the BOC runtime (C4 gate workload).

Measures the dispatch-rate ceiling on a single producer worker for the
``sched-wsq-fanout`` (C4) decision. Each producer behavior runs on a
...
``boc_sched_dispatch`` (``pushed_pending`` then ``pushed_local`` once
``pending`` is occupied). Contention on the producer worker's
per-worker queue back-pointer is the failure mode the C4 N=4 sub-queues
proposal addresses; this benchmark surfaces ``enqueue_cas_retries`` on
the producer worker as the gating signal.
...
def derive_sizes(cfg: FanoutConfig) -> FanoutConfig:
    ...
    a contention-heavy operating point on the C4 gating workload.
...
# ---------------------------------------------------------------------------
# Derived metrics (C4 gate signal)
# ---------------------------------------------------------------------------


def compute_derived_metrics(stats: Optional[list],
                            completed_children: int) -> dict:
    """Compute the C4 gate signal from a per-worker stats snapshot.

    The producer worker is identified as the worker with the largest
    ``pushed_local + pushed_pending`` total over the session. The
    ...
    if not stats:
        return out
    producer_pushes = [
        int(w.get("pushed_local", 0)) + int(w.get("pushed_pending", 0))
        for w in stats
    ]
    ...
    p_pending = int(stats[p_idx].get("pushed_pending", 0))
    ...
    out["producer_pushed_pending"] = p_pending
